...
<div class="contain">
         <h2> Edit Profile</h2>
        <form action="edit_seller_profiile.jsp" method="post">
            <%
                String str1=(String)session.getAttribute("e");
            %>
                <%
                           Class.forName("com.mysql.cj.jdbc.Driver");
			Connection con1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/painting","root","passord.9506");
                      // String str1=(String)session.getAttribute("email");
                        String sql1="select* from seller where email = '"+str1+"'";
            ...
            <input type="text" id="name" name="name" required value="<%=rs1.getString(3)%>"
                                     ><!-- comment -->
            ...
            <input type="text" id="contact_number" name="contact_number" required value="<%=rs1.getString(4)%>">
             ...
             <input type="email" id="email" name="e" required value="<%=rs1.getString(5)%>"
                                     ><!-- comment -->
            ...
            <select onchange="print_city('state', this.selectedIndex);" id="s" name="state"  required class="I2" value="<%=rs1.getString(6)%>"
                                     ></select>
            <select id="state" name="city" required class="I2" value="<%=rs1.getString(7)%>"
                                     ></select>
            <script>
               print_state("s");
            </script>
            ...
            <input type="text" id="Address" name="Address" required value="<%=rs1.getString(8)%>"
                                     >
             <label for="pincode">pincode:</label>
            <input type="text" id="pincode" name="pincode" required value="<%=rs1.getInt(9)%>"
                                     >
...
//    String sql1="insert into customer(customer_name,email,mobile_no,state,city,address,pincode,password,con_password) values('"+a+"','"+b+"', '"+c+"','"+d+"','"+e+"','"+f+"' ,'"+g+"','"+h+"' ,'"+i+"')";
    String sql2="Update seller set seller_name = '"+a+"',contact_no='"+b+"' ,email= '"+e+"', state='"+d+"',city='"+c+"', address ='"+f+"' , pincode = '"+g+"' where  email='"+str1+"'";
